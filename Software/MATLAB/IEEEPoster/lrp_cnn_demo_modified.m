...
chosen_classes = [2 7 10];
for i = I(chosen_classes)
    x = X(i,:,:,:);
  ...
  %  R = nn.lrp(ypred,'epsilon',1.);   %as Eq(58) from DOI: 10.1371/journal.pone.0130140
    ...
    for ys = [3 5 10]
     yselect = (1:size(Y,2) == ys)*1.;
%     %R = nn.lrp(yselect); %compute first layer relevance for an arbitrarily selected class
   R = nn.lrp(yselect,'epsilon',1.);   %as Eq(58) from DOI: 10.1371/journal.pone.0130140
 %    R = nn.lrp(yselect,'alphabeta',2); %compute first layer relevance for an arbitrarily selected class
    %fprintf('Chosen Class: %d\n\n', yselect-1);
    ...
    figure;
    imshow(img); axis off ; title(['Class ' num2str(ys-1)],'FontSize',14); drawnow;
    end
  %  input('hit enter!')
end
