...
def write_dataset(all_args):
    ...
    if not d.dict_file:
        dictf = os.path.join(d.data_directory, d.tag + ".dict")
        f = open(dictf, 'w')
        f.write("gram_dictionaries = %s\n" % repr(p.gram_dictionaries))
        f.write("answer_words = %s\n" % repr(p.answer_words))
        f.write("question_words = %s\n" % repr(p.question_words))
        f.write("number_words_max = %s\n" % repr(p.number_words_max))
        f.close()
        all_args['dict_file'] = dictf
#    all_args['gram_dictionaries'] = p.gram_dictionaries
#    all_args['answer_words'] = p.answer_words
#    all_args['question_words'] = p.question_words
#    all_args['number_words_max'] = p.number_words_max
    file_contents = """
...
def dataset_def(args_d):
    ...
    args = D(do_rotations = True,                # True if the shapes can be rotated
             area_min = 0.004,                   # Minimal area of an object
             area_max = 0.200 / p.object_max,    # Maximal area of an object (scales down with the amount of shapes)
             elong_range = 5.0)
...
#    p.area_value = removeprob(p.area_value, 1)
...
#    p.language_question = {'color': 0, 'shape': 0, 'location_hor': 1, 'location_vert': 0, 'size': 0}
#    p.language_real_question = {'color': 1, 'shape': 0, 'location_hor': 0, 'location_vert': 0, 'size': 0}
    p.language_real_question = {'color': 1, 'shape': 1, 'location_hor': 1, 'location_vert': 1, 'size': 1}
#    p.language_sentence = 'oneofeach'
#    p.language_sentence = 'all'
    ...
    d.data_directory = '/cluster/pauli/data/babyAI/textual_v3/'
    ...
    # Prefix of the files
    if args.do_rotations:
        preprefix = 'BABYAI_gray_'
    else:
        preprefix = 'BABYAI_gray_norot_'
    qtypes = "-".join([str(x[0]) for x in p.language_real_question.items() if x[1]])
    d.tag = preprefix + str(d.n_examples) + 'x' + str(p.object_max) + 'obj_'  + str(p.x_res) + 'x' + str(p.y_res) + '.' + qtypes + '.' + d.tag
